...
class CenterPointHead(nn.Module):
    ...
    def __init__(self,in_channels,num_classes,
                num_task_channel=10,#reg 2,height 1,dim 3,rot 2,vel 2,all 10
                num_heatmap_channel=2,
                shared_conv_channel=64,
                num_shared_convs=2,
                num_seperate_convs=2,
                bn=True,
                init_bias=-2.19):#focal loss trick
        super().__init__()
        self.shared_conv = SeparateHead(in_channels,shared_conv_channel,num_shared_convs,bn)
        self.heatmap_head = SeparateHead(shared_conv_channel, num_classes*num_heatmap_channel, num_seperate_convs,  bn, init_bias=init_bias)
